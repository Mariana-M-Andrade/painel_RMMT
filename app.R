...
sinasc_v2 <- read.csv("sinasc_v2.csv")
...
sinasc_v2$ano <- as.character(sinasc_v2$ano)
  ...
  '),
    ...
    tabItems(
      ...
      tabItem(tabName = "sobre",
        ...
        <h3 style="font-weight: bold;">Autoria</h3>
        <p> Esse painel foi produzido por Mariana Modenesi de Andrade, aluna do curso de Estatística da UFES, como parte do projeto de Iniciação Científica "Caracterização dos casos de morte materna tardia no Brasil". </p> 
      ')
      ),
...
# Calculando a RMM tardia com os inputs
  ...
  dados_rc <- reactive({
    omt_rc <- omt_filtrados() 
    nasc_rc <- nasc_filtrados() 
    ...
    dados_rc <- left_join(omt_rc, nasc_rc, by = c("ano", "uf", "racacor"))
  ...
  dados_idade <- reactive({
    ...
    dados_idade <- full_join(omt, nasc, by = c("ano", "uf"))
